...
class ShippingEstimatorService:
    ...
    def __init__(
        self,
        base_per_km: Decimal = Decimal("0.50"),
        per_kg: Decimal = Decimal("0.10"),
    ...
    def estimate(
        ...
        km = Decimal(str(self._haversine_km(origin, destination))).quantize(
            Decimal("0.01")
        )
        base_cost = (self.base_per_km * km).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
        weight_cost = (self.per_kg * Decimal(str(weight_kg))).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
        multiplier = self.speed_multipliers[speed]
        total = ((base_cost + weight_cost) * multiplier).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP
        )
        return {
